...
const VocabularyTest = ({ wordPairs }) => {
    ...
    const checkAnswers = () => {
        ...
        const newResults = userAnswers.map((answer, index) => {
            if (answer && wordPairs[index]) {
                if (normalizeAnswer(answer) === normalizeAnswer(wordPairs[index].vietnamese)) {
                    correct++;
                    return 'correct';
                } else {
                    incorrect++;
    ...
    return (
        <div>
            <div className="test__container">
                {wordPairs.map((pair, index) => (
                    ...
                    >
                        ...
                        <div className="english-word" onClick={() => copyEnglishWord(pair.english)}>{pair.english}</div>
                        <button
                            className="btn__read"
                            onClick={() => speakWord(pair.english)}
                            tabIndex="-1"
                        >
            ...
            <div className='wp__foot'>
                ...
                <p className='correct'>Correct count: {correctCount}</p>
                <p className='incorrect'>Incorrect count: {incorrectCount}</p>
